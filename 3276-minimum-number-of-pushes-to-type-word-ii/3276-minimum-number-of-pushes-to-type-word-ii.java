...
Algorithm
1. Initialize a frequency vector frequency of size 26 to store the count of each letter in the word.
    - Iterate through each character c in word and increment the count in frequency at the index corresponding to c - 'a'.
2. Sort the frequency vector in descending order to prioritize letters with higher counts.
3. Initialize a variable totalPushes to store the total number of key presses required.
4. Iterate through the sorted frequency vector:
...
5. Return totalPushes as the minimum number of key presses required to type the word.
...
//         Integer[] freqArray = Arrays.stream(frequency).boxed().toArray(Integer[]::new);

//         // Sort frequencies in descending order
//         Arrays.sort(freqArray, (a, b) -> b - a);
//         int totalPushes = 0;

//         // Calculate total number of pushes
//         for (int i = 0; i < 26; i++) {
//             if (freqArray[i] == 0)
//                 return totalPushes; // No more characters to process
//             totalPushes += (i / 8 + 1) * freqArray[i];
//         }
//         return totalPushes;
//     }
// }
...
Algorithm
1. Create a frequency map frequencyMap to store the count of each letter in the input string word.
    - Iterate through word and for each character, increment its count in frequencyMap.
2. Create a priority queue frequencyQueue to store the frequencies of letters in descending order.
    - Iterate through frequencyMap and push each frequency into frequencyQueue.
3. Initialize a variable totalPushes to 0 to keep track of the total number of presses.
4. Initialize an index variable index to 0.
5. Calculate the total number of presses by processing the frequencies in the priority queue.
...
6. Return totalPushes as the minimum number of presses needed.
...
class Solution {
    public int minimumPushes(String word) {
        ...
        // Calculate the total no of pushes
        while(!freqQueue.isEmpty()){
            totalPushes += (index / 8 + 1) * freqQueue.poll();
            index++;
        }
        return totalPushes;
    }
