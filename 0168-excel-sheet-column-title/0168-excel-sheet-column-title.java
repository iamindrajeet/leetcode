...
2. Do the following as long as columnNumber is greater than 0:
    ...
    b. Find the character corresponding to columnNumber % 26 and append it to the ans in the end.
...
The number of operations would be equal to the number of while loop iterations. In each iteration, the number N gets divided by 26. Hence the time complexity would be O(log base 26 * N). Note that the base of the logarithm is not relevant when it comes to big O, since all logarithms are related by a constant factor.
...
class Solution {
    public String convertToTitle(int columnNumber) {
        // StringBuilder to construct the column name
        StringBuilder columnName = new StringBuilder();

        while (columnNumber != 0) {
            columnNumber--; // Adjust column number to 0-based index
            
            // Calculate the last character for the current step
            char lastChar = (char) ((columnNumber % 26) + 'A');
            
            // Append the character to the result
            columnName.append(lastChar);
            
            // Move to the next position in the column title
            columnNumber /= 26;
        }
